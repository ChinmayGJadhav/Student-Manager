# -*- coding: utf-8 -*-
"""
Created on Mon Jun 10 09:04:28 2019
...
def student_new_record():
    ...
    Label(screen4, text="", bg='brown',width='87', height="1").grid(row=1,columnspan=4)
...
def fetch_record(semester):
    Label(screen3, text="", bg='#174873',width='70', height='25').place(x=45, y=170)    
    if semester == '--0--':
...
def student_records():
    ...
    Label(screen3, text="Your Record", width='41', height="2", font=("Calibri", 22, 'bold'),fg='white', bg='#d9660a').grid(row=0, sticky=W, columnspan=4)
    #Label(screen3, text="", bg='#174873', width='60', height='18').place(x=0, y=127)     
    Label(screen3, text="", bg='white',width='88', height="1").grid(row=1,columnspan=4)
...
def welcome_page(student_info):
    ...
    Label(screen2, text="", bg='#174873', width='20', height='20').place(x=0, y=100)
    photo = PhotoImage(file="C:\\Users\\chinmay\\Desktop\\welcome_page.png") # opening left side image - Note: Ifimage is in same folder then no need to mention the full path
    ...
    Button(screen2, text='Check your result', width=20, font=("Open Sans", 13, 'bold'),bg='brown', fg='white', command=student_records ).place(x=270, y=350)# validating the values and inserting new record in database for signup
    
    '''photo1 = PhotoImage(file="background.png") # opening right side image - Note: If image is in same folder then no need to mention the full path
       ...
       if university.get() == "--select your university--": # checking for selection of university
           ...
           if tnc.get(): # checking for acceptance of agreement
               if re.match("^.+@(\[?)[a-zA-Z0-9-.]+.([a-zA-Z]{2,3}|[0-9]{1,3})(]?)$", email.get()):# validating the email
                   if password.get() == repassword.get(): # checking both password match or no # if u enter in this block everything is fine just enter the values in database
                       gender_value = 'male'
                       if gender.get() == 2:
                           gender_value = 'female'
                       connection = pymysql.connect(host="localhost", user="root",passwd="", database="edumate") # database connection
                       ...
                       Button(screen1, text='Proceed to Login ->', width=20, font=("Open Sans", 9,'bold'), bg='brown', fg='white',command=screen1.destroy).place(x=190, y=565) # button to navigate back to login page
                   else:
...
def register():
    global screen1, fullname, email, password, repassword, university, gender, tnc # making
    #all entry field variable global
    fullname = StringVar()
    email = StringVar()
    password = StringVar()
    ...
    Label(screen1, text="Registration Form", width='32', height="2", font=("Calibri", 22,
    'bold'), fg='white', bg='#d9660a').place(x=0, y=0)
    ...
    bg='#174873', anchor=W).place(x=150, y=160)
    Entry(screen1, textvar=fullname).place(x=300, y=160)
    Label(screen1, text="Email ID:", font=("Open Sans", 11, 'bold'), fg='white',
    bg='#174873', anchor=W).place(x=150, y=210)
    Entry(screen1, textvar=email).place(x=300, y=210)
    Label(screen1, text="Gender:", font=("Open Sans", 11, 'bold'), fg='white', bg='#174873',
    anchor=W).place(x=150, y=260)
    Radiobutton(screen1, text="Male", variable=gender, value=1,
    bg='#174873').place(x=300, y=260)    
    Radiobutton(screen1, text="Female", variable=gender, value=2,
    bg='#174873').place(x=370, y=260)
    Label(screen1, text="University:", font=("Open Sans", 11, 'bold'), fg='white',
    bg='#174873', anchor=W).place(x=150, y=310)
    ...
    Label(screen1, text="Password:", font=("Open Sans", 11, 'bold'), fg='white',
    bg='#174873', anchor=W).place(x=150, y=360)
    Entry(screen1, textvar=password, show="*").place(x=300, y=360)
    Label(screen1, text="Re-Password:", font=("Open Sans", 11, 'bold'), fg='white',
    bg='#174873', anchor=W).place(x=150, y=410)
    ...
    bg='#174873', font=("Open Sans", 9, 'bold'), fg='brown').place(x=175, y=450)
    Button(screen1, text='Submit', width=20, font=("Open Sans", 13, 'bold'), bg='brown',
    fg='white', command=register_user).place(x=170, y=490)
    
# login creditentials verification
def login_verify():
    global studentID
    connection = pymysql.connect(host="localhost", user="root", passwd="",
    database="edumate") # database connection
    cursor = connection.cursor()
    select_query = "SELECT * FROM student_details where email = '" +username_verify.get() + "' AND password = '" + password_verify.get() + "';" # queries for
    #retrieving values
    cursor.execute(select_query) # executing the queries
    ...
    connection.close() # closing the connection of the database
    if student_info:
        messagebox.showinfo("Congratulation", "Login Succesfull") # displaying message for
        #successful login
...
def mainscreen():
     ...
     Label(screen, text="EDUMATE - Student Manager", width="500", height="2",
     font=("Calibri", 22, 'bold'), fg='white', bg='#d9660a').pack()
     Label(text="", bg='#174873').pack() # for leaving a space in between
     Label(screen, text="", bg='#174873',width='70', height='25').place(x=45, y=120) # blue
     #background in middle of window
     ...
     13, 'bold'), fg='white',command=login_verify).pack()
     Label(screen, text="", bg='#174873').pack() # for leaving a space in between
     Button(screen, text="New User? Register Here", height="2", width="30", bg='#e79700',
     font=("Open Sans", 10, 'bold'), fg='white',command=register).pack()
     screen.mainloop()
